...
async def list_categories_simple(user: dict = Depends(get_current_user)):
    """Simple list of category names for backward compatibility"""
...
async def create_category(data: CategoryCreate, owner: dict = Depends(require_owner)):
    """Create a new product category (Owner only)"""
    ...
    category = {
        ...
        "color": data.color or "#007BFF",
        "shop_id": owner["shop_id"],
        "created_at": now
    }
    
    await db.categories.insert_one(category)
    return CategoryResponse(**category, product_count=0)
...
async def list_categories(user: dict = Depends(get_current_user)):
    ...
    for cat in categories:
        ...
        result.append(CategoryResponse(**cat, product_count=count))
    
    # Add "Other" category for uncategorized products
    uncategorized_count = await db.products.count_documents({
        "shop_id": user["shop_id"],
        "$or": [{"category": None}, {"category": ""}, {"category": {"$exists": False}}]
    })
    
    if uncategorized_count > 0 or len(categories) == 0:
...
async def get_category(category_id: str, user: dict = Depends(get_current_user)):
    ...
    category = await db.categories.find_one(
        {"id": category_id, "shop_id": user["shop_id"]}, {"_id": 0}
    )
    if not category:
    ...
    return CategoryResponse(**category, product_count=count)

@api_router.put("/categories/{category_id}", response_model=CategoryResponse)
async def update_category(category_id: str, data: CategoryUpdate, owner: dict = Depends(require_owner)):
    """Update a category (Owner only)"""
    if category_id == "other":
        raise HTTPException(status_code=400, detail="Cannot modify the 'Other' category")
    
    # Get old category name for updating products
    old_category = await db.categories.find_one(
        {"id": category_id, "shop_id": owner["shop_id"]}, {"_id": 0}
    )
    if not old_category:
        raise HTTPException(status_code=404, detail="Category not found")
    ...
    # If name is being changed, update all products with old category name
    if "name" in update_data and update_data["name"] != old_category["name"]:
        await db.products.update_many(
            {"shop_id": owner["shop_id"], "category": old_category["name"]},
            {"$set": {"category": update_data["name"]}}
        )
    
    await db.categories.update_one(
        {"id": category_id, "shop_id": owner["shop_id"]},
        {"$set": update_data}
    )
...
async def delete_category(category_id: str, owner: dict = Depends(require_owner)):
    """Delete a category (Owner only) - products become uncategorized"""
    if category_id == "other":
    ...
    await db.products.update_many(
        {"shop_id": owner["shop_id"], "category": category["name"]},
        {"$set": {"category": None}}
    )
...
@api_router.get("/categories/{category_id}/products", response_model=List[ProductResponse])
async def get_category_products(category_id: str, user: dict = Depends(get_current_user)):
    ...
    if category_id == "other":
        ...
        products = await db.products.find(
            {"shop_id": user["shop_id"], "category": category["name"]},
            {"_id": 0}
        ).to_list(1000)
